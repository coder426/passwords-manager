...
function pwdms_add_tinymce_plugin( $plugin_array ) {
	$plugin_array['pmsinsertshortcode'] = PWDMS_JS .'pms-shortcode.js';
	return $plugin_array;
}
...
function pms_front_pass_table($message) { 
	...
	//Front html
	$message = '';
	$message .= '<div id="cname_'.$count.'" class="cname">'.$cate_name.'</div><div class="table-responsive pwdms_table_responsive bg-dark text-white" id="table-responsive" style="max-width:100%;overflow: auto;">';
	$message .=     '<table id="front_pass_table_'.$count.'" class="table table-borderless table-striped pms_data_table">';
	$message .=        '<thead>';
	$message .=           '<tr>';
	foreach($table_header as $header){
		$message .= '<th class="bg-primary text-white">'.$header.'</th>';
	}
	$message .=            '</tr>';
	$message .=          '</thead>';
	$message .=      '</table>';
