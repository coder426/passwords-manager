                ...
                if (typeof response.data.page !== 'undefined') {
                    ...
                    if (response.data.done == false) {
                        ...
                        if (response.data.found_posts > 0) {
                            ...
                            jQuery('#pwdms_csv_export_table').after('<div class="export_successfully"><p>Data export successfully</p></div>');
                        } else {
                            jQuery('.export_successfully').remove();
    ...
    function pwdms_export_csv() {
        jQuery.get(MyAjax.ajaxUrl, {
            ...
            .done(function (response) {
                window.location = MyAjax.ajaxurl + '?action=pwdms_export_csv&document_title=' + jQuery('#pwdms_document_title').val();
            });
    }
});